...
export default function HistoryDetailPage() {
  ...
  const handleEdit = () => {
    ...
    try {
      sessionStorage.setItem("FYP_EDIT_PREFS", JSON.stringify(data.preferences));
    } catch {}
    router.push("/search?from=history");
  };

  if (loading) return <div className={styles.pageSkeleton}>Loading…</div>;
  if (!data) return <div className={styles.pageSkeleton}>Record not found.</div>;
  ...
  const summaryChips = [
    ...
    prefs.hasBudget === "Yes" && prefs.maxTuition ? `≤ $${prefs.maxTuition}` : null,
  ].filter(Boolean);

  return (
    <div className={styles.background}>
      <div className={styles.resultsCol}>
        <div className={styles.ctaRow} style={{ justifyContent: "space-between" }}>
        ...
        <div className={styles.resultCard} style={{ marginBottom: 18, background: "#eaf9ff" }}>
          <h3 className={styles.cardTitle}>{prefs.program || "Unknown Program"}</h3>
          <p className={styles.cardMeta}>
            Saved: {new Date(data.createdAt).toLocaleString()}
            {summaryChips.length ? " · " + summaryChips.join(" · ") : ""}
          </p>
        </div>
        ...
        {(data.recommendations || []).map((r, idx) => (
          ...
          >
            <div
              ...
              aria-hidden="true"
            >
              {typeof r.matchPercentage === "number" ? `${r.matchPercentage}%` : ""}
            </div>

            <h3 className={styles.cardTitle} style={{ paddingRight: 80 }}>
              {r.programName}
            </h3>
            <p className={styles.cardMeta}>
              {r.universityName}
